...
func fetchToken(ctx context.Context, o *Options3LO, v url.Values) (*Token, string, error) {
	...
	// Make request
	logger.DebugContext(ctx, "3LO token fetch", "request", clog.HTTPRequest(req, []byte(v.Encode())))
	...
	logger.DebugContext(ctx, "3LO token response", "response", clog.HTTPResponse(r, body))
