...
func (b *BucketHandle) SignedURL(name string, opts *SignedURLOptions) (string, error) {
	if opts.GoogleAccessID != "" && (opts.SignBytes != nil || len(opts.PrivateKey) > 0) {
		return SignedURL(b.name, name, opts)
	}
	...
	return SignedURL(b.name, name, newopts)
}
