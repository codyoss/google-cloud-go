...
func (c *Client) getETag(ctx context.Context, suffix string) (value, etag string, err error) {
	...
	retryer := newRetryer()
	for {
		logger.Log(ctx, clog.DynamicLevel(), "request to metadata service", "request", clog.HTTPRequest(req, nil))
	...
	logger.Log(ctx, clog.DynamicLevel(), "response from metadata service", "response", clog.HTTPResponse(res, all))
