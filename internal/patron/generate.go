...
func protoc(uc *UnifiedConfig) error {
	...
	stubs := "--go_out=/output/tmp/"
	if strings.Contains(uc.transport, "grpc") {
		stubs = "--go_out=plugins=grpc:/output/tmp/"
	}
	args := []string{"--experimental_allow_proto3_optional", stubs, "-I", "/apis"}
